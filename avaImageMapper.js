...
function AvaImageMapper() {
		...
		return {
			...
			getColoringCode : function(arg) {
				...
				if (arg) {
					...
					utils.foreachReverse(objects, function(x) {
						html_code += utils.encode(x.toColoringString()) + '<br />';
					});
				} else {
					utils.foreachReverse(objects, function(x) {
						html_code += x.toString();
	...
	var code = (function(){
		...
		return {
			print: function(format) {
                switch (format) {
                  case 'labelling':
                    content.innerHTML = app.getLabellingCode(true);
                    break;
                  case 'coloring':
                    content.innerHTML = app.getColoringCode(true);
                    break;
                  default:
	...
	var buttons = (function() {
		var all = utils.id('nav').getElementsByTagName('li'),
			...
			to_labelling = utils.id('to_labelling'),
			to_coloring = utils.id('to_coloring'),
		...
		function onToLabellingButtonClick(e) {
			// Generate html code only
			info.unload();
			code.print('labelling');
			
			e.preventDefault();
		}
		
		function onToColoringButtonClick(e) {
			// Generate html code only
			info.unload();
			code.print('coloring');
		...
		to_labelling.addEventListener('click', onToLabellingButtonClick, false);
		to_coloring.addEventListener('click', onToColoringButtonClick, false);
	...
	Rect.prototype.toColoringString = function() { //to coloring syntax code
		var x2 = this.params.x + this.params.width,
			y2 = this.params.y + this.params.height;
		return '[<area shape="rect" coords="'
			+ this.params.x + ','
	...
	Circle.prototype.toColoringString = function() { //to coloring syntax code
        return '[<area shape="circle" coords="'
			+ this.params.cx + ','
			+ this.params.cy + ','
			+ this.params.radius
	...
	Polygon.prototype.toColoringString = function() { //to coloring syntax code
		for (var i = 0, count = this.params.length, str = ''; i < count; i++) {
			str += this.params[i];
			if (i != count - 1) {
				str += ',';
